...
### 4.1 同一处订正只改了一半（落包后发现，已修）

上面那次订正**第一次只改了 frontmatter**，条目正文 `**置信度说明**` 末句仍写着
「`original_confidence` 的 0.5 是导出管道的默认值，不是作者评估」——
于是**已发布的 `EXP-TS-0001` 里同时存在两个互相矛盾的声明**：字段说 0.9，正文说 0.5。

这与 `EXP-PY-0016`（改 `confidence` 0.90→0.75 时漏改正文说明，见 §六）是**同一个模式**，
两次都发生在「同一个事实在多处登记」的地方。因此本条的处置不止于改对文字：

- 已把正文那句改为「取作者自评的 0.90（源记录 `provenance_weight: 0.8`）」；
- 在 `audit/validate_packs.py` 新增 `check_confidence_note()`，校验 `**置信度说明**`
  与 frontmatter 的取值是否自洽（只报 WARN）。该检查的判别边界是实测定的：
  取值一律出现在 `original_confidence` **之后**，故只向后看一个窗口 ——
  取前后窗口会把上一句的「给 0.90」卷进来，实测**既漏报真缺陷又误报无数字的句子**。
  已用 6 个样例（含修复前原文）验证能捕获、且不误报；
- 全仓 56 条非遗留条目重跑：0 命中 —— 即这个矛盾只此一处。

**教训**：把同一事实写进两个字段，就要么让脚本对读、要么只写一处。
本次两次都是靠人复读发现的，第三次未必还能发现。
