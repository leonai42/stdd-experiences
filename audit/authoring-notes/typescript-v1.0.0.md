...
4. **该订正第一次只改了一半（已修）。** 第一次只改了 frontmatter，正文
   `**置信度说明**` 末句仍写「`original_confidence` 的 0.5 是导出管道的默认值」——
   于是**已发布的条目里并存两个互相矛盾的声明**（字段 0.9 / 正文 0.5）。
   这与 `EXP-PY-0016` 漏改正文说明是同一模式：**同一个事实登记在两处，改一处等于制造矛盾**。

   已改正文；并在 `audit/validate_packs.py` 加 `check_confidence_note()` 对读两处
   （只报 WARN），用 6 个样例（含修复前原文）验证能捕获且不误报。

   本条比第 3 点更值得记：第 3 点是**读错了证据**，本条是**改对了字段却留下了矛盾**。
   两者的共同点是都靠人复读才发现 —— 同一批里出现两次，说明这个环节不能只靠复读。
